...
其他的快捷键还有很多，象 Ctrl+G（跳转到指定行）、Ctrl+F4（关闭当前编辑页面）、Ctrl+F（搜索）等等，这些快捷键由于是各个编辑器都会提供的，而且定义的键位也都差不多，就没什么可说的了；

## 自己常用

### 命名类

ctrl + shift + U 切换全体大小写

shift + alt + u 切换首字母大小写 (命名方式切换)

### 代码自动类

在资源后加 .twr JDK 1.7 之后的 try with resource 代码块
...
CTRL + W 选中整行, 多次按下选择范围变大

CTRL + X   剪切,删除行

Alt + Shift + UP/DOWN 迅速向上/向下挪动整行

Ctrl + Alt + V 生成等号左边的内容

Alt + 回车 自动导入包, 添加异常, 方法添加参数等, 较为实用, 经常用到
