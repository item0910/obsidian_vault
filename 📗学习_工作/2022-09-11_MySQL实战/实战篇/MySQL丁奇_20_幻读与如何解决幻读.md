...
5. T2 时刻，session B 事务提交，写入了两条语句；
6. T4 时刻，session C 事务提交，写入了两条语句；
7. T6 时刻，session A 事务提交，写入了 update t set d=100 where d=5 这条语句。
...
为了支持边读边写（MVCC），而不是阻塞写只能读，更不能破坏一致性的读写 [[MySQL丁奇_06_全局锁和表锁#^458dbc|备份为什么要加锁]]
