...
### 1. 单线程处理网络请求到多线程处理
### 机制:
- 主线程建立Socket 连接后, 阻塞式的将客户端请求交给多个IO线程去读请求, 并交给主线程执行.
- 主线程完成后, 将结果写入缓冲区, 阻塞式的等待IO 线程(多个)将这些结果读取并返回客户端.
- 我的理解: 
	- 主线程轮询请求时, 发现socket请求后, 就将连接建立好, 并放入等待队列中
	- 主线程再次需要处理这个socket时, 就将这个socket分配给另一个IO线程, 让他来负责与客户端连接(这一步有多个IO线程在并行处理), 而主线程阻塞等待
	- 等到这个请求被处理好, 在传递给主线程; 主线程结束阻塞, 执行请求.
		![[附件/image/39_Redis6.0新特性.jpg]]
	- 主线程处理完成后, 会将结果写入缓冲区, 等待IO线程回写socket, 回写完成后, 阻塞的主线程再继续执行队列中的其它命令.
...
- 对于使用RESP2协议的客户端来说, 要想获得失效消息, 就要执行订阅命令, 同时, 再使用一个客户端, 执行CLIENT TRACKING 命令. 设置服务端将失效消息转发给RESP2 的客户端
```
//客户端B执行，客户端B的ID号是303
...
### 4. 启用RESP3通信协议(可以直接从请求的开头字符来实现区分不同的数据类型)
通过不同的开头字符, 来区分更多的数据类型, 方便了数据转换提高了效率. 
另外还支持客户端以普通模式和广播模式实现客户端缓存.
[[🔧生产和其他项目笔记/IT项目/B_02_数据库/Redis实战/未来篇_39-42_6.0新特性NVM和加餐/Redis蒋德钧_42_Redis的信息协议]]
### 总结
